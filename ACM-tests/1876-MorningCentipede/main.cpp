...
int main() {
	...
	a = SLIPPERS + 2 * rightSlippers;      // Выбираются только правые тапочки (пока они не закончатся) + 40 левых тапочек
	b = 2 * leftSlippers + (SLIPPERS - 1); // Выбираются 39 правых тапочек, потом выбираются все левые тапочки, затем отыскивается последняя правая тапочка
